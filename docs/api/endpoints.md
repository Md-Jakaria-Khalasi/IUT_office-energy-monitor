...
> Companion to [`../API_DOCUMENTATION.md`](../API_DOCUMENTATION.md). Read
> that first for the high-level overview; this file is the full catalogue.
...
up — does **not** verify the database.
...
| `acked`  | boolean | —       | When `true`, return acknowledged alerts; default `false`. |
...
Mark an alert as acknowledged. Idempotent — calling twice is a no-op the second time.
...
Returns **404** when the alert id is unknown. The endpoint is **reserved for future use** — the bundled Discord bot's `!ack` command currently only prints to the console and does not call this endpoint yet; see `discord_bot/app/cogs/alerts.py`.
...
| `room`   | string  | —       | Filter to a single room name (e.g. `Drawing Room`).          |
...
allow-list — see [`../PROJECT_DOCUMENTATION.md`](../PROJECT_DOCUMENTATION.md#5-deployment).